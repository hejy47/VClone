...
def read_config():
  ...
  separators = "; . [ ] ( ) ~ ! - + & * / % < > ^ | ? { } = # , \" \\ : $ ' ` @ begin end" #config.get('Language', 'separators').strip('"').split(' ')
...
def tokenize_blocks(file_string, comment_inline_pattern, comment_open_close_pattern, separators, logging, file_path):
  ...
  if '.v' in file_extensions:
    (statement_types, statement_linenos, block_linenos, blocks) = extractVerilogBlock.getBlocks(file_string, logging, file_path)
  ...
  if block_linenos is None:
    ...
    try:
      ...
      for i, block_string in enumerate(blocks):
        ...
        (statement_start_line, statement_end_line) = statement_linenos[i]
        statement_type = statement_types[i]
        ...
        block_stats = (block_hash, block_lines, block_LOC, block_SLOC, start_line, end_line, statement_type, statement_start_line, statement_end_line)
        # Rather a copy of the file string here for tokenization
  ...
  if (project_format == 'zipblocks') or (project_format == 'folderblocks'):
    ...
    try:
      for relative_id, block_data in blocks_data:
        ...
        (block_hash, block_lines, block_LOC, block_SLOC, start_line, end_line, statement_type, statement_start_line, statement_end_line) = blocks_stats
        ...
        FILE_stats_file.write('b' + ','.join([proj_id,block_id,'\"'+block_hash+'\"', str(block_lines),str(block_LOC),str(block_SLOC),str(start_line),str(end_line),statement_type,str(statement_start_line),str(statement_end_line)]) + '\n')
